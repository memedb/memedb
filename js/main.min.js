...
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,c){var b=$jscomp.checkStringArgs(this,a,"startsWith");a+="";for(var e=b.length,f=a.length,h=Math.max(0,Math.min(c|0,b.length)),g=0;g<f&&h<e;)if(b[h++]!=a[g++])return!1;return g>=f}},"es6-impl","es3");toggled=[];searchCode="";
...
function sendCommand(a,b,c,d){var e="",f;for(f in c)c.hasOwnProperty(f)&&(e+=f+"="+c[f]+"&");e+="session="+b;b=new XMLHttpRequest;b.onreadystatechange=function(){4==this.readyState&&200==this.status&&(response=JSON.parse(this.responseText),"success"==response.status&&d(response))};b.open("POST","/api/"+a,!0);b.setRequestHeader("Content-type","application/x-www-form-urlencoded");b.send(e)}
function followAction(){var a=document.cookie.split("; "),b=null;for(i=0;i<a.length;i++){var c=a[i];if(c.startsWith("PHPSESSID")){b=c.split("=")[1];break}}var d=document.getElementById("follow-btn");"follow"!=d.className&&"unfollow"!=d.className||sendCommand(d.className,b,{handle:d.dataset.handle},function(a){d.className=a.following?"unfollow":"follow";d.innerHTML="<span>"+a.followers+"</span>"})}
