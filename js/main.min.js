...
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,c){var b=$jscomp.checkStringArgs(this,a,"startsWith");a+="";for(var e=b.length,g=a.length,f=Math.max(0,Math.min(c|0,b.length)),h=0;h<g&&f<e;)if(b[f++]!=a[h++])return!1;return h>=g}},"es6-impl","es3");toggled=[];searchCode="";
...
function sendCommand(a,b,c,d){if(null==b){var e=document.cookie.split("; ");for(i=0;i<e.length;i++){var g=e[i];if(g.startsWith("PHPSESSID")){b=g.split("=")[1];break}}}var e="",f;for(f in c)c.hasOwnProperty(f)&&(e+=f+"="+c[f]+"&");e+="session="+b;b=new XMLHttpRequest;b.onreadystatechange=function(){4==this.readyState&&200==this.status&&(response=JSON.parse(this.responseText),"success"==response.status&&d(response))};b.open("POST","/api/"+a,!0);b.setRequestHeader("Content-type","application/x-www-form-urlencoded");
b.send(e)}function followAction(){var a=document.getElementById("follow-btn");"follow"!=a.className&&"unfollow"!=a.className||sendCommand(a.className,null,{handle:a.dataset.handle},function(b){a.className=b.following?"unfollow":"follow";a.innerHTML="<span>"+b.followers+"</span>"})}
...
function closeAddLib(){$(".addLib").css({display:"none"});$("#imp-bg-fade").css("display","none")}function openAccount(){$(".s-dropdown").css({display:"block"});$("#imp-bg-fade").css("display","");setTimeout(function(){$(".l-sett-opt").css({opacity:"",right:""});$("#imp-bg-fade").css("opacity","")},10)}function closeAccount(){$(".s-dropdown").css({display:"none"});$("#imp-bg-fade").css("display","none")}
function openPost(){window.scrollTo(0,0);$(".m-post").css({display:"block"});$("#imp-bg-fade").css("display","");setTimeout(function(){$(".l-sett-opt").css({opacity:"",right:""});$("#imp-bg-fade").css("opacity","")},10)}function closePost(){$(".m-post").css({display:"none"});$("#imp-bg-fade").css("display","none")}
function openBalance(){$(".balance-message").css({display:"block"});$("#imp-bg-fade").css("display","");setTimeout(function(){$(".l-sett-opt").css({opacity:"",right:""});$("#imp-bg-fade").css("opacity","")},10);$(".peep").children().css("filter","blur(5px)");$(".balance-message").css("filter","blur(0px)");$(".peep").css("background","#222")}
function closeBalance(){$(".balance-message").css({display:"none"});$("#imp-bg-fade").css("display","none");$(".peep").children().css("filter","blur(0px)");$(".peep").css("background","#fff")}function libDrop(a){};
