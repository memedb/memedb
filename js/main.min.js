...
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,c){var b=$jscomp.checkStringArgs(this,a,"startsWith");a+="";for(var e=b.length,f=a.length,h=Math.max(0,Math.min(c|0,b.length)),g=0;g<f&&h<e;)if(b[h++]!=a[g++])return!1;return g>=f}},"es6-impl","es3");toggled=[];
...
!toggled[this.id]});$(".l-drop").click(function(a){openOptions(this.parentElement.id);a.stopPropagation()});$(".h-more").click(function(a){openOptions(this.parentElement.id);a.stopPropagation()});$("#imp-bg-fade").click(function(){$(".l-sett-opt").css({opacity:"0",right:"50px"});$("#imp-message, #imp-bg-fade").css("opacity","0");setTimeout(function(){$("#imp-message, #imp-bg-fade, .l-sett-opt").css("display","none")},100);closeSettings();closeTagSearch();closeStats();closeAddLib()});$(".openSettings").click(function(){openSettings();
$("#imp-bg-fade").css("opacity","")});$(".closeSettings").click(function(){closeSettings();$("#imp-message, #imp-bg-fade").css("opacity","0")});$(".openTagSearch").click(function(){openTagSearch();$("#imp-bg-fade").css("opacity","")});$(".closeTagSearch").click(function(){closeTagSearch();$("#imp-message, #imp-bg-fade").css("opacity","0")});$(".openStats").click(function(){openStats();$("#imp-bg-fade").css("opacity","")});$(".closeStats").click(function(){closeStats();$("#imp-message, #imp-bg-fade").css("opacity",
"0")});$(".openAddLib").click(function(){openAddLib();$("#imp-bg-fade").css("opacity","")});$(".closeAddLib").click(function(){closeAddLib();$("#imp-message, #imp-bg-fade").css("opacity","0")});$(".s-tab").click(function(){this.classList.contains("s-selected")||($(".s-tab").toggleClass("s-selected"),$(".s-c-wrapper").toggleClass("s-slide"))})});
function sendCommand(a,b,c,d){var e="",f;for(f in c)c.hasOwnProperty(f)&&(e+=f+"="+c[f]+"&");e+="session="+b;b=new XMLHttpRequest;b.onreadystatechange=function(){4==this.readyState&&200==this.status&&d(JSON.parse(this.responseText))};b.open("POST","/api/"+a,!0);b.setRequestHeader("Content-type","application/x-www-form-urlencoded");b.send(e)}
function followAction(){var a=document.cookie.split("; "),b=null;for(i=0;i<a.length;i++){var c=a[i];if(c.startsWith("PHPSESSID")){b=c.split("=")[1];break}}a=document.getElementById("follow-btn");"follow"!=a.className&&"unfollow"!=a.className||sendCommand(a.className,b,{handle:a.dataset.handle},function(a){console.log("response: \n\n"+a)})}
...
function closeAddLib(){$(".addLib").css({display:"none"});$("#imp-bg-fade").css("display","none")};
