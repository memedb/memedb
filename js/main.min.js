toggled=[];
$(document).ready(function(){libs=$(".library");for(i=0;i<libs.length;i++)libs[i].id=i.toString();libs.click(function(a){toggled[this.id]?(this.querySelector(".l-settings").style.transform="",this.nextElementSibling.style.height="0px"):(this.querySelector(".l-settings").style.transform="rotate(180deg)",content=this.nextElementSibling,content.style.height="",height=content.clientHeight,content.style.height="0px",setTimeout(function(){content.style.height=height.toString()+"px"},10));toggled[this.id]=!toggled[this.id]});
$(".l-drop").click(function(a){openOptions(this.parentElement.id);a.stopPropagation()});$(".h-more").click(function(a){openOptions(this.parentElement.id);a.stopPropagation()});$("#imp-bg-fade").click(function(){$(".l-sett-opt").css({opacity:"0",right:"50px"});$("#imp-message, #imp-bg-fade").css("opacity","0");setTimeout(function(){$("#imp-message, #imp-bg-fade, .l-sett-opt").css("display","none")},100);closeSettings();closeTagSearch();closeStats();closeAddLib()});$(".openSettings").click(function(){openSettings();
